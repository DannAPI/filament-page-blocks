...
final class MakeAdminModelCommand extends Command
{
    ...
    public function handle(Filesystem $files): int
    {
        ...
        try {
            $this->addAdminFieldHelpers($files, $relativeName);
    ...
    private function addAdminFieldHelpers(Filesystem $files, string $relativeName): void
    {
        ...
        $path = "{$resourceRoot}/{$directory}/{$modelClass}Resource.php";
        ...
        $contents = $this->wrapGeneratedForm($contents);
        $contents = $this->useAdminFieldHelpers($contents);

        $files->put($path, $contents, true);
        $this->components->info("Enabled admin field helpers in: {$path}");
    }

    private function useAdminFieldHelpers(string $contents): string
    {
        $components = [
            'TextInput' => 'text',
            'Textarea' => 'textarea',
    ...
    private function wrapGeneratedForm(string $contents): string
    {
        ...
        $updated = preg_replace_callback(
            ...
            static function (array $matches): string {
                $fields = trim((string) $matches['fields']);
                $fields = preg_replace('/^ {16}/m', '', $fields) ?? $fields;
                $fields = preg_replace('/^/m', '                        ', $fields) ?? $fields;

                return $matches['prefix'].PHP_EOL
                    .'                static::formLayout('.PHP_EOL
                    .'                    main: ['.PHP_EOL
                    .$fields.PHP_EOL
                    .'                    ],'.PHP_EOL
                    .'                ),'.PHP_EOL
